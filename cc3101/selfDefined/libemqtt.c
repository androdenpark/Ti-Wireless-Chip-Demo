 ...
 *
 * Fork developed by Vicente Ruiz Rodr¨ªguez
...
uint8_t mqtt_num_rem_len_bytes(const uint8_t* buf) {
	...
	//UART_PRINT("mqtt_num_rem_len_bytes\n");
...
int mqtt_connect(mqtt_broker_handle_t* broker)
{
	...
	//uint16_t clientidlen = sizeof(broker->clientid);
        uint16_t clientidlen = strlen(broker->clientid);
	//uint16_t usernamelen = sizeof(broker->username);
        uint16_t usernamelen = strlen(broker->username);
	//uint16_t passwordlen = sizeof(broker->password);
  ...
  //  uint8_t fixed_header[fixedHeaderSize];
    uint16_t fixed_header_len = fixedHeaderSize*sizeof(uint8_t);
    uint8_t *fixed_header = (uint8_t *)malloc(fixed_header_len);
	...
	//uint8_t packet[sizeof(fixed_header)+sizeof(var_header)+payload_len];
        uint16_t packet_len = (fixed_header_len+sizeof(var_header)+payload_len)*sizeof(uint8_t);
	...
	// Send the packet
	if(packet_len > (broker->send(broker->socket_info, packet, packet_len))){
                free(packet);
                free(fixed_header);
                UART_PRINT("CONN:Send the packet error!\n");
		return -1;
	}
        ...
        UART_PRINT("CONN:Send the packet OK!\n");
...
int mqtt_publish_with_qos(mqtt_broker_handle_t* broker, const char* topic, const char* msg, uint8_t retain, uint8_t qos, uint16_t* message_id) {
	...
	// Variable header
        uint16_t var_header_len=(topiclen+2+qos_size)*sizeof(uint8_t);
	uint8_t *var_header=(uint8_t *)malloc(var_header_len); // Topic size (2 bytes), utf-encoded topic
	...
	uint8_t fixedHeaderSize = 2;    // Default size = one byte Message Type + one byte Remaining Length
	uint16_t remainLen = var_header_len+msglen;
	if (remainLen > 127){
		fixedHeaderSize++;          // add an additional byte for Remaining Length
	}
        uint16_t fixed_header_len = fixedHeaderSize*sizeof(uint8_t);
   ...
   if (remainLen <= 127){
	...
	uint8_t packet_len = (fixed_header_len+var_header_len+msglen)*sizeof(uint8_t);
        uint8_t *packet = (uint8_t *)malloc(packet_len);
        //uint8_t packet[fixed_header_len+var_header_len+msglen];
	memset(packet, 0, packet_len);
	memcpy(packet, fixed_header, fixed_header_len);
	memcpy(packet+fixed_header_len, var_header, var_header_len);
	memcpy(packet+fixed_header_len+var_header_len, msg, msglen);
...
int mqtt_subscribe(mqtt_broker_handle_t* broker, const char* topic, uint16_t* message_id) {
	...
	//uint8_t utf_topic[topiclen+3]; // Topic size (2 bytes), utf-encoded topic, QoS byte
        uint16_t utf_topic_len = (topiclen+3)*sizeof(uint8_t);
        uint8_t *utf_topic = (uint8_t *)malloc(utf_topic_len);
	...
	//uint8_t packet[sizeof(var_header)+sizeof(fixed_header)+utf_topic_len];
        uint16_t packet_len = (sizeof(var_header)+sizeof(fixed_header)+utf_topic_len)*sizeof(uint8_t);
        uint8_t *packet = (uint8_t *)malloc(packet_len);
...
int mqtt_unsubscribe(mqtt_broker_handle_t* broker, const char* topic, uint16_t* message_id) {
	...
	//uint8_t utf_topic[topiclen+2]; // Topic size (2 bytes), utf-encoded topic
                uint16_t utf_topic_len = (topiclen+2)*sizeof(uint8_t);
        uint8_t *utf_topic = (uint8_t *)malloc(utf_topic_len);
