 ...
 */
bStatus_t ExampleService_AddService( uint32 services )
{
...
bStatus_t ExampleService_GetParameter( uint8 param, void *value )
{
  ...
  switch ( param )
  {
    case SIMPLEPROFILE_CHAR1:
#ifdef HAL_UART_TRANS
	   VOID osal_memcpy( value, simpleProfileChar1, SIMPLEPROFILE_CHAR1_LEN );  //simpleProfileChar1 ÊÇÊý×éÃû
#else
       *((uint8*)value) = simpleProfileChar1;	//simpleProfileChar1 ÊÇ±äÁ¿
#endif
      break;
...

{
    ...
    switch ( uuid )
    {
      ...
      case SIMPLEPROFILE_CHAR3_UUID:
        ...
        if ( offset == 0 )
        {
			...
			{				  						//ÎªÊ²Ã´²»ÊÇÆäËû characteristic  (´í)
				status = ATT_ERR_INVALID_VALUE_SIZE;
			}
			#else	//¶Ô·½(Central)ÓÃ´®¿Ú·¢ËÍÊý¾Ý¸öÊýµÄËæÒâµÄ(ÔÝÊ±ÈËÎªÔ¼¶¨Ð¡ÓÚµÈÓÚ20 SIMPLEPROFILE_CHAR1_LEN)
					//Èô£¾20¾ÍÈÎÎñ´íÁË£¬ÒòÎªÎÒµÄattribute tableÖÐµÄbufferÖ»ÓÐ SIMPLEPROFILE_CHAR1_LEN ´ó
					//(Ò²¿ÉÒÔ²»ÓÃÅÐ¶Ï£¬ÎÒÖ»½ÓÊÕ20¸öÒ»´Î)
					//»¹¿ÉÒÔÔÚCentral¶Ë·¢ËÍµÄÊ±ºòÅÐ¶ÏÏÂ£¬¶àÓÚ20¸ö¾Í²»·¢²¢ÇÒÌáÊ¾´íÎó ÕâÑù±È·¢¹ýÀ´½ÓÊÕÁË²»´¦Àí¸üºÃ
					//»¹¿ÉÒÔÔÚCentral¶Ë°Ñ¶àÓÚ20×Ö½ÚµÄÇé¿ö·Ö¶à´Î·¢ËÍ ÔÝÊ±²»¿¼ÂÇÕâÖÖÇé¿ö
			if ( len > SIMPLEPROFILE_CHAR1_LEN )   //len µÈÓÚ¶àÉÙÊÇÓÉË­¾ö¶¨µÄ £¬ÊÇsimpleProfileAttrTblÖ®characteristic 1Ö®Characteristic Value³¤¶È¾ö¶¨µÄ  (´í)
			{				  						//ÎªÊ²Ã´²»ÊÇÆäËû characteristic  (´í)
				status = ATT_ERR_INVALID_VALUE_SIZE;
			}
        ...
        if ( status == SUCCESS )
        {
		  ...
		  //Õâ¸ölenÔÚ±¾ÎÄ¼þµÄÆäËûº¯ÊýÒ²ÄÜÓÃµ½±ÈÈçGetParameter
		  //ÔÚ±¾º¯ÊýµÄ×îºó»Øµ÷º¯ÊýÒ²ÄÜÓÃµ½£¬Ëä²»ÄÜ´«²ÎÊý£¬µ«ÊÇ¿ÉÓÃÈ«¾Ö±äÁ¿
          ...
          if( pAttr->pValue == simpleProfileChar1 )		//µÈÓÚsimpleProfileChar1ÊÇÓÉË­¾ö¶¨µÄ
          													...
          													//(ÎªÊ²Ã´²»ÊÇÆäËûcharacteristic 2¡¢3¡¢4¡¢5£¬ÔõÑù²ÅÄÜÓÃÉÏÕâÐ©characteristic)
          													//ÔÚÕâÀïÊÇÊý×éÃû simpleProfileChar1
          {
            notifyApp = SIMPLEPROFILE_CHAR1;    //³ýÁËchar1ºÍchar3»¹ÓÐÆäËûµÄ°É£¬ÌØ±ðÊÇchar5     
          }
          else
      ...
      case GATT_CLIENT_CHAR_CFG_UUID:			//¸ÉÊ²Ã´ÓÃµÄ? ÄÜ·ñ±»Ö´ÐÐµ½ ?
  ...
  {
    simpleProfile_AppCBs->pfnSimpleProfileChange( notifyApp );  //µ÷ÓÃ»Øµ÷º¯Êý simpleProfileChangeCB()
  }
